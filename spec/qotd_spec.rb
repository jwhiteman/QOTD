...
describe Qotd do
  ...
  describe Qotd::Strategy::Serial do
    def config
  ...
  describe Qotd::Strategy::ProcessPerRequest do
    def config
      @config ||= config_dummy.tap do |config|
        config.strategy = Qotd::Strategy::ProcessPerRequest
        config.verbose  = false
        config.port     = 10018
      end
    end
  ...
  describe Qotd::Strategy::ThreadPerRequest do
    def config
      @config ||= config_dummy.tap do |config|
        config.strategy = Qotd::Strategy::ThreadPerRequest
        config.verbose  = false
        config.port     = 10019
      end
    end
  ...
  describe Qotd::Strategy::Prefork do
    def config
      @config ||= config_dummy.tap do |config|
